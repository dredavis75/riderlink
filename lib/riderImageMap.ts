// Single source of truth for local rider-logo keyword matching.
// Used by both app/components/ProductImage.tsx (client-side, checked first)
// and app/api/product-image/route.ts (server-side fallback tier) — these
// used to be two separate, drifting lists with different priority order,
// which was the main cause of photos landing on the wrong rider item.

// Most-specific keywords FIRST — first match wins
export const LOCAL_IMAGE_MAP: [string[], string][] = [
...
export function getLocalImage(name: string): string | null {
